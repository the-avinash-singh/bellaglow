...
function  page() {
  return (
    <div className="max-w-360 mx-auto lg:flex min-h-screen">
      ...
      <Breadcrumb className='m-4 mb-0 text-lg'/>
      <Menu/>
      </div>
    <main className='flex-1 px-4 py-8 sm:px-6 lg:px-10 lg:py-10'>
      ...
      {products.map((product, index)=>(
        <Product2 key={index+product.name} name={product.name} desc={product.desc} img={product.img} price={product.price} tag={product.tag} route={product.route} />
      ))}
      </div>
    </main>
    </div>
